 ...
 *	Matrix operations
 *
 *	Replace element i in row N with i ^ N
 *
 */
...
int main (int argc, char** argv) {
    ...
    cl_mem mem_input;                     // memory object
	cl_mem mem_output;                     // memory object
	...
	for (i = 0; i < M; i++) {
		for (j = 0; j < N; ++j) {
			inpooot[i * M + j] = rand() % 6 + i;
			printf("\t%d", inpooot[i * M + j]);
		}
		printf("\n");
	}
	printf("\n");
    ...
    //
    mem_input = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * M * N, NULL, NULL);
	mem_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * M * N, NULL, NULL);
    if (!mem_input) {
    ...
    //
    err = clEnqueueWriteBuffer(commands, mem_input, CL_TRUE, 0, sizeof(int) * M * N, inpooot, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err |= clSetKernelArg(kernel, 1, sizeof(unsigned int), &M);
...
//		perror("--");
    ...
    //
    err = clEnqueueReadBuffer(commands, mem_output, CL_TRUE, 0, sizeof(int) * M * N, outpoot, 0, NULL, NULL );
    if (err != CL_SUCCESS) {
